...
object UniSelectQuery3 {
  def main(args: Array[String]): Unit = {
    ...
    val oll = spark.sql("select * from ol where orderline.productid = '6515' limit 5")

    oll.createOrReplaceTempView("oll")
    oll.show()

    val dfh = spark.sql("select f.personid, f.feedback from dfn join f on f.personid = qid join oll on oll.personid = qid join o on o.PersonId = qid where f.feedback like '%1.0%' and o.OrderDate = '2022-06-16'")    
    dfh.show()
    

    val end = System.currentTimeMillis
    printf("Query 3 Run Time = %f[s]\n", (end - start) / 1000.0)
    printf("with dfn as (match (:person)-[:knows]-(q:person) return q.id as qid),\n with o as (mongodb.unibench.orders),\n with ol as (mongodb.unibench.orders.orderline),\n with oll as (select * from ol where orderline.productid = '6515' limit 5),\n with f as (hbase.default.feedback),\n select f.personid, f.feedback from dfn join f on f.personid = qid join oll on oll.personid = qid join o on o.PersonId = qid where f.feedback like '1.0' and o.OrderDate = '2022-06-16'\n")
