...
object UniSelectQuery2 {
  def main(args: Array[String]): Unit = {
    ...
    val oll = ol.filter("orderline.productid = 2675").limit(5)
//    val oll = spark.sql("select * from ol where orderline.productid = '2675' limit 5")
    oll.createOrReplaceTempView("oll")
    oll.show()

    val dfh = spark.sql("select f.personid, f.feedback from dfn join f on f.personid = qid join oll on oll.personid = qid join o on o.PersonId = qid where OrderDate = '2018-07-07'")
    dfh.show()
